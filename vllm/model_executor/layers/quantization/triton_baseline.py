...
class TritonBaselineLinearMethod(LinearMethodBase):
    ...
    def create_weights(self, layer, input_size_per_partition, output_partition_sizes, input_size, output_size, params_dtype, **extra_weight_attrs):
        ...
        local_input_size = input_size_per_partition if isinstance(input_size_per_partition, int) else sum(input_size_per_partition)

        weight = torch.nn.Parameter(torch.empty(local_output_size, local_input_size, dtype=params_dtype))
        ...
        if extra_weight_attrs.get("bias", False):
            bias = torch.nn.Parameter(torch.empty(local_output_size, dtype=params_dtype))
    ...
    def apply(self, layer, x, bias=None) -> torch.Tensor:
        out = triton_matmul(x, layer.weight.t().contiguous())
        if bias is not None:
            out += bias
        return out
...
class TritonBaselineEmbeddingMethod(LinearMethodBase):
    ...
    def create_weights(self, layer, input_size_per_partition, output_partition_sizes, input_size, output_size, params_dtype, **extra_weight_attrs):
        ...
        weight = torch.nn.Parameter(torch.empty(local_vocab_size, local_hidden_size, dtype=params_dtype))
